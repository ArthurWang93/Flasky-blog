...
def post(id):
    ...
    page = request.args.get('page', 1, type=int)
    if page == -1:
        page = (post.comments.count() - 1)/current_app.config['FALSKY_COMMENTS_PER_PAGE'] + 1
    pagination = post.comments.order_by(Comment.timestamp.asc()).paginate(page,
                                                                          per_page=current_app.config['FALSKY_COMMENTS_PER_PAGE'],
                                                                          error_out=False)
...
def follow(username):
    ...
    if current_user.is_following(user):
        flash('you are already following this guy')
        return redirect(url_for('.user', username=username))
    current_user.follow(user)
    db.session.commit()
    flash("You are noe following %s" % username)
    return redirect(url_for('.user', username=username))
...
def moderate_enable(id):
    comment = Comment.query.get_ot_404(id)
